...
from neural_clbf.controllers_mps.controller_mps import Controller
from neural_clbf.datamodules.episodic_datamodule import EpisodicDataModule
from neural_clbf.experiments_mps import ExperimentSuite


class NeuralObsBFControllerMPS(LightningModule, Controller):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        # super(NeuralObsBFControllerMPS, self).__init__(
    ...
    def u_(
        ...
        # exploratory = exploratory.to(u_exploratory.device)
        u_exploratory = u_exploratory.to(u.device)

        u[exploratory] = u_exploratory[exploratory]
        u = u.reshape(x.shape[0], -1)
        costs[goal_seeking] = gs_cost[goal_seeking]

        exp_cost = exp_cost.to(costs.device)
    ...
    def find_duplicate_indices(self, lst, val, idx=0):
        # Base case: if the list is empty, return an empty list
        if not lst:
            return []

        # Recursive case: if the first element of the list is equal to the value,
        # return a list containing the current index plus the result of the function
        # called on the rest of the list; otherwise, return the result of the function
        # called on the rest of the list
        return ([idx] if lst[0] == val else []) + self.find_duplicate_indices(lst[1:], val, idx + 1)
    
    def torch_unique_implementation(self, u_options: torch.Tensor) -> torch.Tensor:

        u_options_lst = u_options.tolist()

        # print('u_options_lst: ', u_options_lst)

        all_duplicates = []

        for i in range(u_options.shape[0]):

            # Use the recursive function to find the indices of duplicates of the value
            duplicate_indices_to_remove = self.find_duplicate_indices(u_options_lst, u_options_lst[i])

            # print('duplicate_indices_to_remove: ', duplicate_indices_to_remove)

            all_duplicates = all_duplicates + duplicate_indices_to_remove[:-1]

            # u_options_first_half = u_options[:i]
            # u_options_first_second_half = u_options[i+1:]

        # print('all_duplicates: ', all_duplicates)

        # Create a mask that is True for elements you want to keep
        mask = torch.ones(len(u_options), dtype=bool)
        # mask[duplicate_indices_to_remove[:-1]] = False
        mask[all_duplicates] = False

        # Create a new tensor that does not include the elements at the specified indices
        u_options = u_options[mask]
    ...
    def lookahead(
        ...
        # print('u_options before: ', u_options)
        # print('u_options 0 element: ', u_options[0])
        # print('u_options before shape: ', u_options.shape)
        # print('u_options before shape 0: ', u_options.shape[0])

        # Remove duplicate elements
        u_options = self.torch_unique_implementation(u_options)


        # print('u_options after: ', u_options)
        # print('u_options 0 element: ', u_options[0])
        # print('u_options after shape: ', u_options.shape)
        # print('u_options after shape 0: ', u_options.shape[0])

        # u_options = torch.unique(u_options, dim=0)
        # # u_options = torch.unique(u_options)
    ...
    def u_goal_seeking(
        ...
        # print('u before clamp: ', u)
        # print('upper_limit: ', upper_limit)
        # print('lower_limit: ', lower_limit)

        # make sure the limits are on the right device
        lower_limit = lower_limit.to(u.device)
        upper_limit = upper_limit.to(u.device)
    ...
    def u_exploratory(
        ...
        # make sure the limits are on the right device
        # lookahead_dual_penalty_mps = self.lookahead_dual_penalty.to(costs.device)
        h_nexts_mps = h_nexts.to(costs.device)
        # h_alpha_mps = self.h_alpha.to(costs.device)
        h_mps = h.to(costs.device)
        ...
        costs.add_(
            self.lookahead_dual_penalty
            * F.leaky_relu(h_nexts_mps - (1 - self.h_alpha) * h_mps, negative_slope=0.001)
        )
        ...
        # make sure the limits are on the right device
        hit_points_h_mps = self.hit_points_h.to(costs.device)
        ...
        costs.add_(
            P
            * F.leaky_relu(
                (h_nexts_mps - hit_points_h_mps[idxs[:, 0]]).abs() - eps_h,
                negative_slope=0.01,
            )
        )

        # make sure the limits are on the right device
        V_nexts_mps = V_nexts.to(costs.device)
        V_mps = V.to(costs.device)

        # # Add a cost to encourage Lyapunov decrease when possible
        # costs.add_(Q * (V_nexts - (1 - self.V_lambda) * V))

        # Add a cost to encourage Lyapunov decrease when possible
        costs.add_(Q * (V_nexts_mps - (1 - self.V_lambda) * V_mps))

        # make sure the limits are on the right device
        u_options = u_options.to(costs.device)
